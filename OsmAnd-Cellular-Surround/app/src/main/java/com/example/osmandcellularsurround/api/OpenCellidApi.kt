...
object OpenCellidApi {
    ...
    suspend fun donateData(
        ...
        return withContext(Dispatchers.IO) {
            try {
                // Determine `act` parameter from radio
                val act = when (radio.lowercase()) {
                    "lte" -> "LTE"
                    "umts", "wcdma" -> "UMTS"
                    "gsm" -> "GSM"
                    "cdma" -> "CDMA"
                ...
                val maskedUrl = url.replace("key=$apiKey", "key=****")
                logger("Donate Request: GET $maskedUrl")
                ...
                logger("Donate Response Code: ${response.code}")
                logger("Donate Response Body: ${responseBody ?: "null"}")

                return@withContext response.isSuccessful
            } catch (e: Exception) {
                logger("Donate Exception: ${e.message}")
                e.printStackTrace()
